        ...
        if country in ["CA", "MX", "AU"]:
            if rates.get("usps_priority_intl") and rates["usps_priority_intl"].get("amount") is not None:
                shopify_rates.append({
                    ...
                    "total_price": str(int(rates["usps_priority_intl"]["amount"] * 100)),
                    "currency": "USD",
            ...
            if rates.get("ups_worldwide") and rates["ups_worldwide"].get("amount") is not None:
                shopify_rates.append({
                    ...
                    "total_price": str(int(rates["ups_worldwide"]["amount"] * 100)),
                    "currency": "USD",
            ...
            if rates.get("no_rush") and rates["no_rush"].get("amount") is not None:
                shopify_rates.append({
                    ...
                    "total_price": str(int(rates["no_rush"]["amount"] * 100)),
                    "currency": "USD",
            ...
            if rates.get("ups_ground") and rates["ups_ground"].get("amount") is not None:
                shopify_rates.append({
                    ...
                    "total_price": str(int(rates["ups_ground"]["amount"] * 100)),
                    "currency": "USD",
            ...
            if rates.get("usps_priority") and rates["usps_priority"].get("amount") is not None:
                shopify_rates.append({
                    ...
                    "total_price": str(int(rates["usps_priority"]["amount"] * 100)),
                    "currency": "USD",
...
def estimate_shipping():
    try:
        ...
        to_address = {
            "postal_code": zip_code,
            "country": "US"  # Estimator only supports US zip-based input
        }
